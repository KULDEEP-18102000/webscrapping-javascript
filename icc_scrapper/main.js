const url = "https://2021.t20worldcup.com/fixtures/men";
...
function extractAllLinks(html) {
    ...
    let scorecardElems = $("a.match-block__link");
    for (let i = 0; i < scorecardElems.length; i++) {
        let link = $(scorecardElems[i]).attr("href");
        let fullLink = "https://2021.t20worldcup.com" + link;
...
function extractMatchDetails(html) {
    ...
    for (let i = 0; i <(allRows.length-2); i++){
        let allCols = $(allRows[i]).find("td.table-body__cell.mc-scorecard__cell.strong");
